...
function package_application() {
  cd "${SRC_DIR}" || exit
  zip -r "${BUILD_DIR}/application.zip" "." -x "*__pycache__*" ".pytest_cache"
  cd - || exit
}
...
function upload_s3() {
  ...
  aws s3 cp --recursive --exclude "*__pycache__*" --exclude ".pytest_cache" "${PROJECT_ROOT}/src" "${BUCKET_URL}/src"
